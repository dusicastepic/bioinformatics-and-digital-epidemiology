...
#setwd('C:\\Users\\dusic\\Desktop\\secondo ANNO\\DIGITAL EPIDEMIOLOGY')


library(TCGAbiolinks)
library(SummarizedExperiment)

proj <- "TCGA-UCEC"#"TCGA-BRCA"

dir.create(file.path(proj))

rna.query.C <- GDCquery(project = proj, data.category = "Transcriptome Profiling", data.type = "Gene Expression Quantification", workflow.type = "HTSeq - FPKM", sample.type = "Primary solid Tumor")
...
rna.query.N <- GDCquery(project = proj, data.category = "Transcriptome Profiling", data.type = "Gene Expression Quantification", workflow.type = "HTSeq - FPKM", sample.type = "Solid Tissue Normal")
